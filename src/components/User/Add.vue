<template>
  <q-card :style="{
    ...
    <q-card-section class="q-pt-lg">
      ...
      :oninput="(str) => Utils.onlyLettersAndNumbersandSpace(str)"
            :rules="[
              (val) =>
                Utils.onlyLettersAndNumbersandSpace(val) ||  $t(Utils.getKey('Only character, numbers and space'))
            ]"
      dense
      outlined
      maxlength="20" />
    ...
    <q-card-section class="q-pt-none">
      <form autocomplete="off">
        <q-input :autofocus="true" type="email" aria-autocomplete="none" autocomplete="off" v-model="user.email"
          :label="$t(Utils.getKey('Email Id'))" dense outlined  required maxlength="20"
...
async function onSubmit() {
  ...
  try {
    ...
    if (!Utils.onlyLettersAndNumbersandSpace(user.value.name)) {
      $q.notify({
        position: "top-right",
        type: "negative",
