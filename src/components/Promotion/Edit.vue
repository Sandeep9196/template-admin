    ...
    <q-card-section class="q-pt-lg">
      <q-form ref="refForm">
        <div class="row">
          ...
          <!-- <div class="row"> -->
            <div
              class="col-12 col-md-6 q-pr-md q-mt-sm"
              v-for="lang in languages"
              :key="lang.locale"
            >
              <label class="text-uppercase">{{ $t(lang?.locale) }}</label>
              <q-input
                v-model="promotion.translates[lang.locale].name"
                :label="$t(Utils.getKey('name'))"
                dense
                autogrow
                type="textarea"
    ...
    <q-card-section class="q-pt-none">
      <div class="row">
        <div class="col-12 col-md-12 q-pr-md">
          <q-radio v-model="promotion.status" val="active" :label="$t('active')"/>
          <q-radio v-model="promotion.status" val="inactive" :label="$t('inactive')"/>
        </div>
      </div>
    </q-card-section>
...
async function onSubmit() {
  ...
  languages.value.forEach((lg, index) => {
    if (promotion.value.translates[lg.locale].name == "") {
      allFill = true;
    }
  });
      ...
      languages.value.forEach((lg, index) => {
        if (promotion.value.translates[lg.locale].name != "") {
          ...
          fomrData.append(
            ...
            promotion.value.translates[lg.locale].name
          );
        }
      });
