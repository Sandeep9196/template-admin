        ...
        <div class="col-12 col-md-5 q-pr-md">
          <div class="d-flex">
            ...
            <div v-for="cat in categoryOption" :key="cat.id">
              <q-chip
                :color="$q.dark.isActive ? 'white' : 'gray'"
...
const langID = computed(() =>
  languages.value.filter((lng) => lng.locale == locale.value)
);
// function
  ...
  languages.value.forEach((lg) => {
    if (
      ...
      if (props.data.translations[lg.locale]) {
        objLang = {
          id: props.data.translations[lg.locale]
            ? props.data.translations[lg.locale].desc_id
            : "",
          language_id: lg.id,
    ...
    if (
      ...
      if (props.data.translations[lg.locale]) {
        objLang2 = {
          id: props.data.translations[lg.locale]
            ? props.data.translations[lg.locale].id
            : "",
          language_id: lg.id,
          field_name: "name",
...
const updateLanguage = () => {
  ...
  languages.value.forEach((lg) => {
    ...
    products.value.name_tran[lg.id] = props.data.translations[lg.locale]?.name;
  });
  // promotion
  let promo = [];
