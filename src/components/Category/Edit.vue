<template>
  ...
  >
    <q-bar dark class="bg-primary text-white">
      <span class="text-body2">{{ $t(Utils.getKey("Edit Category")) }}</span>
      <q-space />
      <q-btn
        dense
        flat
    ...
    <q-card-section class="q-pt-none">
      ...
      <div class="row" v-for="lang in languages" :key="lang.locale">
        <div class="col-12 col-md-6 q-pr-md ">
          ...
          <q-input
            v-model="category.translates[lang.locale].name"
            :label="$t(Utils.getKey('name'))"
            dense
            autogrow
            lazy-rules
        ...
        <div class="col-12 col-md-6 q-pr-md">
          <label class="text-uppercase">{{ $t( lang.locale) }}</label>
          <q-input
            v-model="category.translates[lang.locale].description"
    ...
    <q-card-section class="q-pt-lg">
      ...
      <div v-if="cropper == ''" class="box-2 mt-1 img-result">
    ...
    <q-card-section class="q-pt-none">
      <div class="row">
        <div class="col-12 col-md-12 q-pr-md" >
          <q-radio v-model="category.status" :disable="category_isusing" val="active" :label="$t('active')" />
          <q-radio v-model="category.status" :disable="category_isusing"  val="inactive" :label="$t('inactive')" />
        </div>
        <div v-if="category_isusing" class="red">
          {{$t('category_in_using_cannot_update_status')}}
        </div>
      </div>
    </q-card-section>
...
const $q = useQuasar();
const { saving, update, getUsing } = useCategory();
...
async function onSubmit() {
  try {
    ...
    languages.value.forEach((lg, index) => {
      if (
        category.value.translates[lg.locale].name == "" ||
        category.value.translates[lg.locale].description == "" ||
        category.value.translates[lg.locale].name == "undefined" ||
        category.value.translates[lg.locale].description == "undefined"
      ) {
        allFill = true;
      }
    });
    ...
    if (cropper.value) {
      cropper.value.getCroppedCanvas().toBlob(async (blob) => {
        ...
        languages.value.forEach((lg, index) => {
          if (category.value.translates[lg.locale].name != "") {
          ...
          if (category.value.translates[lg.locale].description != "") {
            if (lg.id == 1) {
              fomrData.append(
                "description",
                category.value.translates[lg.locale].description
...
onMounted(async () => {
  ...
  let checkUsing = await getUsing(category.value.id)
