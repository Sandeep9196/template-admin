            ...
            <div class="row mt-3">
              ...
              <div class="col-12 q-mt-lg col-md-12 q-pr-md">
                ...
                <q-editor
                  ...
                  :rules="[val => (!!val && val !== '<br>') || $t(Utils.getKey('Field is required'))]"
...
const { add, getType, getGroup } = useMenu();
...
const onLoadDataGroup = async () => {
  ...
  groupTemp.value = gGroup.data.map((g) => {
    return {
      id: g.id,
...
onMounted(async () => {
  languages.value.forEach((lg) => {
    menu.value.title[lg.id] = "";
    menu.value.content[lg.id] = "";
