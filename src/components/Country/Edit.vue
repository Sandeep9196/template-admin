        ...
        <!-- <div class="row">
          <div class="col-12 col-md-12 q-pr-md">
            <q-input
              ...
              :oninput="(evt) => Utils.containsNumbersAndCaracterEvent(evt)"
...
async function onSubmit() {
  try {
    ...
    await update(country.value.id, {
      ...country.value,
      translation_name: currencyData,
    });
    emit("onUpdated");
    $q.notify({
      position: "top-right",
...
onMounted(() => {
  languages.value.forEach((lg) => {
    country.value.translation_name[lg.locale] =
      country.value?.translates[lg.locale]?.name || "";
  });
});
</script>
