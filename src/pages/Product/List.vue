<template>
  ...
  <q-dialog v-model="showDelete" persistent>
    <confirm
      ...
      :message="Utils.getKey('disable_product')"
...
const getSubCategory = async () => {
  if (filters.category_id) {
    ...
    subCategoryOption.value = tempSubCategoryOptions = getSub.data.map(
      (pro) => {
        return {
          id: pro.id,
          name: pro?.translates[locale.value]?.name,
    ...
    subCategoryOption.value = tempSubCategoryOptions = [];
  }
};
watch(
  () => filters.category_id,
...
const onDelete = async () => {
  try {
    await trash(editData.value.id, editData.value);
    onRefresh();
    setTimeout(() => {
      $q.notify({
        position: "top-right",
        type: "positive",
        icon: "fas fa-check",
        message: t(Utils.getKey("delete successfully")),
      });
    }, 1000);
