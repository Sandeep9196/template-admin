<template>
  <q-card style="margin-left: 0px; box-shadow: none; min-height: 85vh">
            ...
            <div class="col-12 col-md-6 q-pr-md">
              <div v-for="wallet in walletData" :key="wallet.code">
                ...
                <span class="text-bold">
                  {{ wallet.code }} {{ wallet.name }} {{ $t(Utils.getKey("wallet payment")) }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-12 col-md-6 q-pr-md">
              <div v-for="wallet in paymentData" :key="wallet.name">
                <q-toggle v-model="wallet.status" size="lg" icon="mdi-currency-usd" />
                <span class="text-bold">
                  {{ wallet.code }} {{ wallet.name }} {{ $t(Utils.getKey("payment Gateway")) }}
                </span>
              </div>
            </div>
          </div>
    ...
    <q-card-section class="text-left q-mt-md">
      <q-btn  outline color="negative" class="q-px-md red" @click="resetCodeBasic" :disable="saving">{{
        $t(Utils.getKey("Reset"))
      }}</q-btn>
      <q-btn color="accent" outline class="q-px-md q-ml-md green" @click="onSubmit"
...
const { loading, getPaymentConfiguration, addPaymentConfiguration } = useConfiguration();
