      ...
      >
        <template v-slot:top>
          ...
          :autofocus="true"
            ...
            :label="$t(Utils.getKey('order status'))"
            dense
            emit-value
            map-options
            option-value="id"
            :option-label="it => $t(Utils.getKey(it.name))"
          />
        ...
        <template v-slot:body-cell-deals_count="props">
          <q-td>
            <span class="font_16 blue mr-2">
              {{
                props.row.order_product
                  ? props.row.product_slot_deals.length
                  : 0
              }}</span
            >
...
const getStatusIcon = (props) => {
  if (props.row.status === "confirmed" || props.row.status === "loser") {
